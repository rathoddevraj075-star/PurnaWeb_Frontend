        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-gray-400 text-sm mt-1">Manage your catalog and SEO performance</p>
                </div>
                <Link
            ...
            {/* Filters */}
            <div className="bg-[#0f1218] rounded-2xl p-5 border border-white/5 shadow-xl shadow-black/20">
                <div className="flex flex-wrap gap-4">
                    <form onSubmit={handleSearch} className="flex-1 min-w-[200px]">
                        <div className="relative group">
                            <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-emerald-400 transition-colors" size={18} />
                            <input
                                type="text"
                                value={search}
                                onChange={(e) => setSearch(e.target.value)}
                                placeholder="Search products..."
                                className="w-full pl-10 pr-4 py-2.5 bg-white/5 border border-white/10 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:border-emerald-500/50 focus:bg-white/10 transition-all"
                            />
                        </div>
                    </form>
                    <select
                    ...
                    >
                        <option value="" className="bg-gray-900 text-gray-300">All Status</option>
                    ...
                    <select
                        ...
                        className="bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-gray-300 focus:outline-none focus:border-emerald-500/50 cursor-pointer hover:bg-white/10 transition-colors"
                    >
                        <option value="" className="bg-gray-900">All SEO</option>
                        <option value="high" className="bg-gray-900">Good (80+)</option>
                        <option value="medium" className="bg-gray-900">Needs Work (50-80)</option>
                        <option value="low" className="bg-gray-900">Poor (&lt;50)</option>
                    </select>
                </div>
            </div>
                                ...
                                <tr key={product._id} className="hover:bg-white/5 transition-colors group">
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-4">
                                            {product.images?.[0]?.url ? (
                                                <img src={product.images[0].url} alt="" className="w-12 h-12 rounded-lg object-cover ring-1 ring-white/10" />
                                            ) : (
                                                <div className="w-12 h-12 bg-white/5 rounded-lg flex items-center justify-center ring-1 ring-white/10">
                                    ...
                                    <td className="px-6 py-4">
                                        <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium border ${product.status === 'published' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' :
                                            ...
                                            <span className={`w-1.5 h-1.5 rounded-full ${product.status === 'published' ? 'bg-emerald-400 animate-pulse' :
                ...
                {data?.pagination && (
                    <div className="flex items-center justify-between px-6 py-4 border-t border-white/5 bg-white/[0.02]">
                        <p className="text-sm text-gray-500">
                            Showing <span className="font-medium text-white">{((page - 1) * 20) + 1}</span> to <span className="font-medium text-white">{Math.min(page * 20, data.pagination.total)}</span> of <span className="font-medium text-white">{data.pagination.total}</span> products
                        </p>
                        <div className="flex gap-2">
                            <button
                                onClick={() => setSearchParams(p => { p.set('page', String(page - 1)); return p; })}
                                disabled={page <= 1}
                                className="p-2 text-gray-400 hover:text-white hover:bg-white/5 rounded-lg disabled:opacity-50 disabled:hover:bg-transparent transition-colors"
                            >
                                <ChevronLeft size={20} />
                            </button>
                            <button
